	...
	var setToMultipleID = function(id, thingToDo, param) {
		multipleSize.forEach(function(size) {
			if (thingToDo == 'innerHTML') {
				...
				document
					.getElementById(id + size)
					.addEventListener('click', param);
	...
	var run = function() {
			...
			for (var i = 0; i < examplesButton.length; i++) {
				examplesButton[i].parentElement.onclick = function() {
					...
					if (isExamplesDisplayed) {
						document.getElementById('exampleList').style.display =
							'block';
						document.getElementsByClassName(
							'wrapper'
						)[0].style.width = 'calc(100% - 400px)';
					} else {
						document.getElementById('exampleList').style.display =
							'none';
						document.getElementsByClassName(
		...
		var markDirty = function() {
			...
			setToMultipleID(
				'safemodeToggle',
				'innerHTML',
				'Safe mode <i class="fa fa-check-square" aria-hidden="true"></i>'
			);
		};
			...
			if (xhr.responseText.indexOf('class Playground') !== -1) {
				...
				if (!filename) {
					...
					window.location.href =
						...
						'//' +
						location.host +
						location.pathname.replace(filename, 'ts.html') +
						window.location.hash;
					return false;
				}
			} else {
				...
				if (filename === 'ts.html') {
					window.location.href =
						...
						'//' +
						location.host +
						location.pathname.replace(filename, 'index.html') +
						window.location.hash;
					return false;
				}
			}
		...
		var loadScriptsList = function() {
			...
			if (exampleList.className != 'typescript') {
				xhr.open(
					'GET',
					'https://raw.githubusercontent.com/BabylonJS/Documentation/master/examples/list.json',
					true
				);
			} else {
				xhr.open(
					'GET',
					'https://raw.githubusercontent.com/BabylonJS/Documentation/master/examples/list_ts.json',
					true
				);
			}
					...
					if (xhr.status === 200) {
						...
						if (exampleList) {
							for (var i = 0; i < scripts.length; i++) {
								...
								var exampleCategory = document.createElement(
									'div'
								);
								exampleCategory.classList.add(
									'categoryContainer'
								);

								var exampleCategoryTitle = document.createElement(
									'p'
								);
								exampleCategoryTitle.innerText =
									scripts[i].title;
								exampleCategory.appendChild(
									exampleCategoryTitle
								);

								for (
									var ii = 0;
									ii < scripts[i].samples.length;
									ii++
								) {
									...
									var exampleImg = document.createElement(
										'img'
									);
									exampleImg.src = scripts[i].samples[
										ii
									].icon.replace(
										'icons',
										'https://doc.babylonjs.com/examples/icons'
									);
									exampleImg.setAttribute(
										'onClick',
										"document.getElementById('PGLink_" +
											scripts[i].samples[ii].PGID +
											"').click();"
									);

									var exampleContent = document.createElement(
										'div'
									);
									exampleContent.classList.add('itemContent');
									exampleContent.setAttribute(
										'onClick',
										"document.getElementById('PGLink_" +
											scripts[i].samples[ii].PGID +
											"').click();"
									...
									var exampleContentLink = document.createElement(
										'div'
									);
									exampleContentLink.classList.add(
										'itemContentLink'
									);

									var exampleTitle = document.createElement(
										'h3'
									);
									exampleTitle.classList.add(
										'exampleCategoryTitle'
									);
									exampleTitle.innerText =
										scripts[i].samples[ii].title;
									var exampleDescr = document.createElement(
										'div'
									);
									exampleDescr.classList.add('itemLineChild');
									exampleDescr.innerText =
										scripts[i].samples[ii].description;

									var exampleDocLink = document.createElement(
										'a'
									);
									exampleDocLink.classList.add(
										'itemLineDocLink'
									);
									exampleDocLink.innerText = 'Documentation';
									exampleDocLink.href =
										scripts[i].samples[ii].doc;
									exampleDocLink.target = '_blank';

									var examplePGLink = document.createElement(
										'a'
									);
									examplePGLink.id =
										'PGLink_' + scripts[i].samples[ii].PGID;
									examplePGLink.classList.add(
										'itemLinePGLink'
									);
									...
									exampleContentLink.appendChild(
										exampleTitle
									);
									exampleContentLink.appendChild(
										exampleDescr
									);
									exampleContent.appendChild(
										exampleContentLink
									);
							...
							var noResultContainer = document.createElement(
								'div'
							);
							noResultContainer.id = 'noResultsContainer';
							noResultContainer.classList.add(
								'categoryContainer'
							);
						...
						// 本剧本地持久化数据重新加载主题
						var theme =
							localStorage.getItem('bjs-playground-theme') ||
							'light';
						toggleTheme(theme);
		...
		var createNewScript = function() {
			...
			let iCanClear = checkSafeMode(
				'Are you sure you want to create a new playground?'
			);
			if (!iCanClear) return;
			location.hash = '';
			currentSnippetToken = null;
			...
			if (monacoMode === 'javascript') {
				...
				jsEditor.setValue(
					'// You have to create a class called Playground. This class must provide a static function named CreateScene(engine, canvas) which must return a BABYLON.Scene object\r\n// You must at least define a camera inside the CreateScene function\r\n\r\nclass Playground {\r\n\tpublic static CreateScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): BABYLON.Scene {\r\n\t\tvar scene = new BABYLON.Scene(engine);\r\n\r\n\t\tvar camera = new BABYLON.FreeCamera("camera1", new BABYLON.Vector3(0, 5, -10), scene);\r\n\t\tcamera.setTarget(BABYLON.Vector3.Zero());\r\n\t\tcamera.attachControl(canvas, true);\r\n\r\n\t\treturn scene;\r\n\t}\r\n}'
				);
			}
		...
		var clear = function() {
			// check if checked is on
			let iCanClear = checkSafeMode(
				'Are you sure you want to clear the playground?'
			);
			if (!iCanClear) return;
			location.hash = '';
			currentSnippetToken = null;
		...
		var checkSafeMode = function(message) {
			...
			if (safeToggle.classList.contains('checked')) {
				...
				if (!confirm) {
					...
					document
						.getElementById('safemodeToggle1600')
		...
		var showNoMetadata = function() {
			...
			if (currentSnippetDescription) {
				document.getElementById(
					'saveFormDescription'
				).value = currentSnippetDescription;
			...
			document.getElementById('saveFormButtonOk').style.display =
				'inline-block';
		};
		showNoMetadata();
		...
		// Run Button 点击运行按钮后触发的函数
		compileAndRun = function() {
			try {
				// 关闭Babylon.js最开始的加载界面
				...
				if (
					...
					BABYLON.Engine.LastCreatedScene.debugLayer.isVisible()
				) {
					showInspector = true;
				}
				...
				getRunCode(jsEditor, function(code) {
					...
					if (!createSceneFunction) {
						...
						// 执行javsscript代码？
						eval(
							'runScript = function(scene, canvas) {' + code + '}'
						);
						...
						//一般正常流程
						//execute the code
						eval(code); // 创造了createScene函数
						//create engine，调用了之前定义的createDefaultEngine()函数
						eval('engine = ' + createEngineFunction + '()');
						if (!engine) {
							showError(
								'createEngine function must return an engine.',
								null
							);
							return;
						}
					...
					engine.runRenderLoop(function() {
						...
						fpsLabel.style.right =
							...
							(jsEditor._domElement.clientWidth +
								canvas.clientWidth) +
							'px';
		...
		var addTexturesToZip = function(zip, index, textures, folder, then) {
			...
			if (
				textures[index].isRenderTarget ||
				textures[index] instanceof BABYLON.DynamicTexture ||
				textures[index].name.indexOf('data:') !== -1
			) {
			...
			if (textures[index].isCube) {
				if (
					textures[index]._extensions &&
					textures[index].name.indexOf('dds') === -1
				) {
					for (var i = 0; i < 6; i++) {
						textures.push({
							name:
								textures[index].name +
								textures[index]._extensions[i]
						});
					}
				} else {
		...
		var addImportedFilesToZip = function(
			zip,
			index,
			importedFiles,
			folder,
		...
		// Fullscreen
		document.getElementById('renderCanvas').addEventListener(
		...
		var toggleEditor = function() {
			...
			if (editorButton.classList.contains('checked')) {
				...
				setToMultipleID(
					'editorButton',
					'innerHTML',
					'Editor <i class="far fa-square" aria-hidden="true"></i>'
				);
			} else {
				...
				setToMultipleID(
					'editorButton',
					'innerHTML',
					'Editor <i class="fa fa-check-square" aria-hidden="true"></i>'
				);
			}
			engine.resize();
		...
		/**
		 * Toggle the dark theme，网站选择主题皮肤
		 */
		var toggleTheme = function(theme) {
			...
			// 代码缩略图
			editorOptions.minimap.enabled = document
				.getElementById('minimapToggle1600')
		...
		var toggleMinimap = function() {
			...
			if (minimapToggle.classList.contains('checked')) {
				...
				setToMultipleID(
					'minimapToggle',
					'innerHTML',
					'Minimap <i class="far fa-square" aria-hidden="true"></i>'
				);
			} else {
				...
				setToMultipleID(
					'minimapToggle',
					'innerHTML',
					'Minimap <i class="fa fa-check-square" aria-hidden="true"></i>'
				);
			}
		...
		var save = function() {
			...
			if (document.getElementById('saveLayer')) {
				currentSnippetTitle = document.getElementById('saveFormTitle')
					.value;
				currentSnippetDescription = document.getElementById(
					'saveFormDescription'
				).value;
				currentSnippetTags = document.getElementById('saveFormTags')
					.value;
			}
			...
			xmlHttp.onreadystatechange = function() {
				if (xmlHttp.readyState === 4) {
					if (xmlHttp.status === 200) {
						...
						showError(
							'Unable to save your code. It may be too long.',
							null
						);
					}
				}
		...
		document
			.getElementById('saveFormButtonOk')
			.addEventListener('click', function() {
				document.getElementById('saveLayer').style.display = 'none';
				save();
			});
		document
			.getElementById('saveFormButtonCancel')
			.addEventListener('click', function() {
				document.getElementById('saveLayer').style.display = 'none';
			});
		// document.getElementById('mainTitle').innerHTML =
			// 'v' + BABYLON.Engine.Version;
		...
		setToMultipleID('safemodeToggle', 'click', function() {
			...
			if (
				...
				setToMultipleID(
					'safemodeToggle',
					'innerHTML',
					'Safe mode <i class="far fa-square" aria-hidden="true"></i>'
				);
			}
		});
		// Editor
