...
(function() {
  ...
  var run = function() {
            ...
            if (exampleList) {
              for (var i = 0; i < scripts.length; i++) {
                ...
                for (var ii = 0; ii < scripts[i].samples.length; ii++) {
                  ...
                  exampleImg.src = scripts[i].samples[ii].icon.replace(
                    "icons",
      ...
      try {
        ...
        getRunCode(jsEditor, function(code) {
          ...
          if (!createSceneFunction) {
            ...
            if (!scene) {
              showError(
                createSceneFunction + " function must return a scene.",
                null
              );
              return;
            }
    ...
    var addTexturesToZip = function(zip, index, textures, folder, then) {
      ...
      if (textures[index].isCube) {
        if (
          ...
          for (var i = 0; i < 6; i++) {
            textures.push({
              name: textures[index].name + textures[index]._extensions[i]
            });
          }
        } else {
    ...
    var save = function() {
      ...
      xmlHttp.open(
        "POST",
        snippetV3Url + (currentSnippetToken ? "/" + currentSnippetToken : ""),
        true
      );
            ...
            xmlHttp.onreadystatechange = function() {
              if (xmlHttp.readyState === 4) {
                if (xmlHttp.status === 200) {
                  ...
                  blockEditorChange = true;
                  jsEditor.setValue(
                    JSON.parse(snippet.jsonPayload).code.toString()
                  );
